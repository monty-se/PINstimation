...
```{r Example.4.2, results = F, eval=FALSE}
show(estimate.vpin)
```
...
```
We run a IVPIN estimation on preloaded dataset `hfdata` of 100 000 observations with `timebarsize` of `5` minutes (`0` seconds).

```{r Example.4.3, eval=FALSE}
estimate.ivpin <- ivpin(hfdata, timebarsize = 60)
```

## [+] IVPIN Estimation started.
##   |-[1] Checking and preparing the data...
##   |-[2] Creating 60-second timebars...[~ 3 seconds]
##   |-[3] Calculating Volume Bucket Size (VBS) and Sigma(DP)...
##   |-[4] Breaking up large 60-second timebars' volume...
##   |-[5] Assigning 60-second timebars into buckets...
##   |-[6] Balancing timebars and adjusting bucket sizes to VBS...
##   |-[7] Calculating aggregate bucket data...
...
We classify data using the LR algorithm with a time lag of `500000` microseconds (`0.5 s`), using the function aggregate_data().
