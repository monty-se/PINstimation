...
We run a IVPIN estimation on preloaded dataset `hfdata` of 100 000 observations with `timebarsize` of `5` minutes (`300` seconds).

```{r Example.4.4, eval=FALSE}
estimate.ivpin <- ivpin(hfdata, timebarsize = 300)
```
